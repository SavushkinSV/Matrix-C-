...
START_TEST(test_s21_sum_matrix_1) {
  matrix_t A = {};
  matrix_t B = {};
  ck_assert_int_eq(s21_sum_matrix(&A, &B, NULL), RETURN_ERROR_MATRIX);
}
END_TEST
...
START_TEST(test_s21_sum_matrix_2) {
  ...
  ck_assert_int_eq(s21_sum_matrix(&A, &B, &result), RETURN_ERROR_CALCULATION);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
...
START_TEST(test_s21_sum_matrix_6) {
  ...
  s21_init_matrix(&expected, 2, 2);
  ck_assert_int_eq(s21_sum_matrix(&A, &B, &result), RETURN_OK);
...
START_TEST(test_s21_sum_matrix_8) {
  ...
  s21_init_matrix(&expected, 4, 3.752365);
  ck_assert_int_eq(s21_sum_matrix(&A, &B, &result), RETURN_OK);
  ck_assert_int_eq(s21_eq_matrix(&result, &expected), SUCCESS);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
...
TCase *tcase_s21_sum_matrix(void) {
  ...
  tcase_add_test(tcase, test_s21_sum_matrix_7);
  tcase_add_test(tcase, test_s21_sum_matrix_8);
