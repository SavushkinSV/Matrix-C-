...
1. [Введение](#введение)
2. [Матрица](#матрица) \
    2.1 [Структура матрицы на языке C](#структура-матрицы-на-языке-c) \
    2.2 [Операции над матрицами](#операции-над-матрицами) \
    2.3 [Создание матриц (create_matrix)](#создание-матриц-create_matrix) \
    2.4 [Очистка матриц (remove_matrix)](#очистка-матриц-remove_matrix) \
    2.5 [Сравнение матриц (eq_matrix)](#сравнение-матриц-eq_matrix) \
    2.6 [Сложение (sum_matrix) и вычитание матриц (sub_matrix)](#сложение-sum_matrix-и-вычитание-матриц-sub_matrix) \
    2.7 [Умножение матрицы на число (mult_number). Умножение двух матриц (mult_matrix)](#умножение-матрицы-на-число-mult_number-умножение-двух-матриц-mult_matrix) \
    2.8 [Транспонирование матрицы (transpose)](#транспонирование-матрицы-transpose) \
    2.9 [Определитель матрицы (determinant)](#определитель-матрицы-determinant) \
    2.10 [Обратная матрица (inverse_matrix)](#обратная-матрица-inverse_matrix)
...
1. Клонируйте этот репозиторий через:
    - SSH `git@github.com:SavushkinSV/Matrix_C.git`
    - HTTPS `https://github.com/SavushkinSV/Matrix_C.git`
2. При необходимости измените базу кода.
3. Запустите `make test` для сборки проекта и запуска тестов.
4. Запустите `make s21_matrix.a`, чтобы создать статическую библиотеку.
5. Запустите `make gcov_report` для формирования отчёта gcov в виде html страницы.

Для корректной работы unit-тестов потребуется библиотека Check. Для установки библиотеки Check в Ubuntu / Linux Mint / Debian используется команда `sudo apt install check`.

Для формирования отчёта gcov в виде html страницы потребуется библиотека lcov. Для установки библиотеки lcov в Ubuntu / Linux Mint / Debian используется команда `sudo apt install lcov`.